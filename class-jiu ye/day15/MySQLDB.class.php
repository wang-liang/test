...
class MysqlDB{
	...
	static function GetInstance($config){
		...
		if(!(self::$instance instanceof self)) //代替上一行的方法，这种方法最常见
		{
			self::$instance = new self($config);//创建对象并保存起来
		}
	...
	function GetOneRow($sql){
		...
		//这里开始处理数据，以返回数组。此时$result是一个结果集
	...
	function GetRows($sql){
		$result = $this->query($sql);
		//$result = self::query($sql);
		//这里开始处理数据，以返回数组。此时$result是一个结果集，且是（多行数据）
		$arr = array(); //空数组，用于存放要返回的结果数组
		while ($rec = mysql_fetch_assoc($result)){ //这里fetch一行数据后返回一个数组
